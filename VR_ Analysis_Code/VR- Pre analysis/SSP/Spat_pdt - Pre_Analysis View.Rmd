...
long_d <- long_d %>% mutate(view_pdt = t03_pdt0) %>%
                   select(id,condition,gruppe,view_pdt,t01_pdt0,t02_pdt0)

aov_view_pdt <- long_d %>% filter(condition == "View") %>% droplevels()

#Creata df to test if there is a SSP within effect
aov_oa <- aov_view_pdt %>% select(id,gruppe,condition,t01_pdt0,t02_pdt0) %>% mutate(view_pdt = (t01_pdt0 + t02_pdt0)/2) %>%
                              select(id,gruppe,condition,view_pdt)

aov_oa$condition <- car::recode(aov_oa$condition, "'View' = 't01 + t02/2'")  
aov_bind <- bind_rows(aov_view_pdt,aov_oa) %>% select(id,gruppe,condition,view_pdt)
aov_view_pdt <- aov_bind

aov_view_pdt$condition <- car::recode(aov_view_pdt$condition, "'View' = 'Spat Sum'")  
aov_view_pdt <- aov_view_pdt %>% dplyr::select(id,gruppe,condition,view_pdt)
...
nmv <- long_d %>% dplyr::filter(condition == "View") %>% dplyr::mutate(diff = view_pdt-((t01_pdt0 + t02_pdt0)/2))
...
aov_view_pdt %>% dplyr::group_by(gruppe) %>%
        rstatix::t_test(
         view_pdt ~ condition, paired = TRUE, 
         p.adjust.method = "none")

```

```{r}
aov_view_pdt %>% dplyr::filter(gruppe == "HC") %>%
        rstatix::wilcox_test(
         view_pdt ~ condition, paired = TRUE, 
         p.adjust.method = "none")
# No differences
