---
title: "CPM_pdt - Pre Analysis Baseline"
author: "JR"
date: "23.06.2023"
...
aov_baseline_pdt <- long_d %>% dplyr::mutate(baseline_pdt = t05_pdt0) %>%
                                        dplyr::select(id,gruppe,condition,baseline_pdt,t01_pdt0)

aov_baseline_pdt <- aov_baseline_pdt %>% dplyr::filter(condition == "Baseline") %>% droplevels()


aov_oa <- aov_baseline_pdt %>% dplyr::select(id,gruppe,condition,t01_pdt0) %>% mutate(baseline_pdt = t01_pdt0) %>%
                              dplyr::select(id,gruppe,condition,baseline_pdt)

aov_oa$condition <- car::recode(aov_oa$condition, "'Baseline' = 'Dom'")  
  
  
aov_bind <- dplyr::bind_rows(aov_baseline_pdt,aov_oa) %>% select(id,gruppe,condition,baseline_pdt)

aov_baseline_pdt <- aov_bind
aov_baseline_pdt$condition <- car::recode(aov_baseline_pdt$condition, "'Baseline' = 'Cpm'")
aov_baseline_pdt
```
...
nmv <- long_d %>% filter(condition == "Baseline") %>% dplyr::mutate(diff = t05_pdt0 - t01_pdt0) %>% dplyr::select(id,gruppe,condition,diff)
...
# No extreme outliers
# One extreme outlier regarding differences in CP group
# CP differences not normal distributed
# CP-dom not normal distributed
```
...
aov_baseline_pdt %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(
         baseline_pdt ~ condition, paired = TRUE,p.adjust.method = "none")
```

```{r}
aov_baseline_pdt %>% dplyr::filter(gruppe == "CP")%>%
        rstatix::wilcox_test(
         baseline_pdt ~ condition, paired = TRUE,p.adjust.method = "none")
# No differences
```
...
tresult <- aov_baseline_pdt %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(baseline_pdt ~ condition, paired = TRUE, p.adjust.method = "none")
...
aov_baseline_pdt %>% dplyr::filter(id != "VR_027") %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(
         baseline_pdt ~ condition, paired = TRUE,p.adjust.method = "none")
# Differences in the CP group turn significant!
```
