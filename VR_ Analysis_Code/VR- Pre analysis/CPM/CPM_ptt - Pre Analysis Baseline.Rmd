---
title: "CPM_ptt - Pre Analysis Baseline"
author: "JR"
date: "23.06.2023"
...
aov_baseline_ptt <- long_d %>% dplyr::mutate(baseline_ptt = t05_ptt) %>%
                                        dplyr::select(id,gruppe,condition,baseline_ptt,t01_ptt)

aov_baseline_ptt <- aov_baseline_ptt %>% dplyr::filter(condition == "Baseline") %>% droplevels()


aov_oa <- aov_baseline_ptt %>% dplyr::select(id,gruppe,condition,t01_ptt) %>% mutate(baseline_ptt = t01_ptt) %>%
                              dplyr::select(id,gruppe,condition,baseline_ptt)

aov_oa$condition <- car::recode(aov_oa$condition, "'Baseline' = 'Dom'")  
  
  
aov_bind <- dplyr::bind_rows(aov_baseline_ptt,aov_oa) %>% select(id,gruppe,condition,baseline_ptt)

aov_baseline_ptt <- aov_bind
aov_baseline_ptt$condition <- car::recode(aov_baseline_ptt$condition, "'Baseline' = 'Cpm'")
aov_baseline_ptt
```
...
nmv <- long_d %>% filter(condition == "Baseline") %>% dplyr::mutate(diff = t05_ptt - t01_ptt) %>% dplyr::select(id,gruppe,condition,diff)
...
aov_baseline_ptt %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(
         baseline_ptt ~ condition, paired = TRUE,p.adjust.method = "none")
```
...
tresult <- aov_baseline_ptt %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(baseline_ptt ~ condition, paired = TRUE, p.adjust.method = "none")
