---
title: "CPM_ptt - Pre Analysis View"
author: "JR"
date: "23.06.2023"
...
aov_view_ptt <- long_d %>% dplyr::mutate(view_ptt = t05_ptt) %>%
                                        dplyr::select(id,gruppe,condition,view_ptt,t01_ptt)

aov_view_ptt <- aov_view_ptt %>% dplyr::filter(condition == "View") %>% droplevels()


aov_oa <- aov_view_ptt %>% dplyr::select(id,gruppe,condition,t01_ptt) %>% mutate(view_ptt = t01_ptt) %>%
                              dplyr::select(id,gruppe,condition,view_ptt)

aov_oa$condition <- car::recode(aov_oa$condition, "'View' = 'Dom'")  
  
  
aov_bind <- dplyr::bind_rows(aov_view_ptt,aov_oa) %>% select(id,gruppe,condition,view_ptt)

aov_view_ptt <- aov_bind
aov_view_ptt$condition <- car::recode(aov_view_ptt$condition, "'View' = 'Cpm'")
aov_view_ptt
```
...
nmv <- long_d %>% filter(condition == "View") %>% dplyr::mutate(diff = t05_ptt - t01_ptt) %>% dplyr::select(id,gruppe,condition,diff)
...
# No  outliers
# No extreme otliers regarding differences
...
aov_view_ptt %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(
         view_ptt ~ condition, paired = TRUE,p.adjust.method = "none")
```
...
tresult <- aov_view_ptt %>% dplyr::group_by(gruppe)%>%
        rstatix::t_test(view_ptt ~ condition, paired = TRUE, p.adjust.method = "none")
